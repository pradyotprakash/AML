...
def readData(folder, inputD=None, outputD=None):
	...
	for imfile in glob.glob('*.png'):
		...
		image = misc.imresize(misc.imread(imfile), [160, 160])
...
def main(inputD, outputD, numLayers, widths, activationFunction, lossFunction, learningRate):
	...
	batches = trainImages.shape[0]/100
	for epoch in xrange(1000):
		print 'Epoch: %s'%epoch
		for i in xrange(batches):
			# print 'Batch: %s'%i
			xs = trainImages[100*i: 100*(i+1)]
			ys = trainLabels[100*i: 100*(i+1)]
			sess.run(trainStep, feed_dict={x: xs, yOriginal: ys})

	correctPredication = tf.equal(tf.argmax(y, 1), tf.argmax(yOriginal, 1))
	accuracy = tf.reduce_mean(tf.cast(correctPredication, tf.float32))

	ret =  100.0*sess.run(accuracy, feed_dict={x: testImages, yOriginal: testLabels})
	sess.close()
